...
int main(u64 arg1, u64 arg2, u64 arg3, u64 arg4, u64 arg5)
{
  ...
  // Unmapping 10 PAGES FROM FRONT (vm_Area count will be same)
  ...
  // Existing vm_area end should be updated(Expanded) (vm_Area count will be same)
  for(int i =1; i < total; i=i+2)
  {
  ...
  // Unmapping 10 PAGES FROM BACK  (vm_Area count will be same)
  for(int i =1; i < total; i = i + 2)
  {
      char* vm_area_start = mm1 + (i * (pages*100));
  ...
  // Deallocating all the Middle of vm_area with READ PROT. Count should be double of READ VM_AREA
  // ONLY Vm area (PROT_WRITE) will be left out
  for(int i =1; i < total; i = i + 2)
  {
  ...
  // All the vm_area with PROT_WRITE will be merged together.
  for(int i =1; i < total;  i=i+2)
  {
